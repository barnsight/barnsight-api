...
def test_admin_dashboard(authorized_client, mock_mongo_client):
  ...
  mock_users_db["farmers"].count_documents.return_value = 3
  mock_users_db["staff"].count_documents.return_value = 5
  ...
  assert data["users"]["farmers"] == 3
  assert data["users"]["staff"] == 5
...
def test_change_user_role(authorized_client, mock_mongo_client):
  mock_db = mock_mongo_client.get_database("users")
  mock_db.list_collection_names.return_value = ["farmers"]

  user_data = {"username": "user1", "role": "farmers", "scopes": ["farmer"]}
  mock_db["farmers"].find_one.return_value = user_data

  response = authorized_client.patch("/api/v1/admin/users/user1/role", json={"new_role": "edge"})

  assert response.status_code == 200
  assert response.json()["message"] == "User user1 role updated from farmers to edge"
